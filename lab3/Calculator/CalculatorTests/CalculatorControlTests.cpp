...
struct CalculatorControlFixture : CalculatorControlDependencies
{
	...
	void ExecuteÑommand(const string& command)
	{
		input = stringstream();
		BOOST_CHECK(input << command);
		BOOST_CHECK(CalculatorControl.HandleCommand());
...
{
	ExecuteÑommand("var identifier_123");
...
BOOST_AUTO_TEST_CASE(cant_assign_value_to_variable_with_incorrect_name)
{
	...
	VerifyCommandHandling("let èìÿ=x", "Failed to assign value to variable\n");
}
...
BOOST_AUTO_TEST_CASE(cant_declare_function_with_incorrect_name)
{
	...
	VerifyCommandHandling("fn èìÿôóíêöèè=x", "Failed to declare function\n");
	...
	VerifyCommandHandling("fn èìÿôóíêöèè=x+x", "Failed to declare function\n");
}
